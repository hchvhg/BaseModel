...
import com.bsoft.job.MyTestJob;
...
public class QuartzConfig {
    ...
    */
    ...
    private void addmyTestJob(Scheduler scheduler) {
        ...
        String cron = "0 0/1 * * * ?";// 定时的时间设置，每1分钟执行一次。表达式在线生成器：http://cron.qqe2.com/
        String className = MyTestJob.class.getName();
        ...
        try {
            ...
            if (checkExist == null) {
                ...
                Trigger trigger = TriggerBuilder.newTrigger().withIdentity(jobName, jobGroup)
                    .withSchedule(cronScheduleBuilder.withMisfireHandlingInstructionIgnoreMisfires()).build();
